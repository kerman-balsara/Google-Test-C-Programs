...
static atomic_int NextAuditNumber;
static atomic_int NextId;

static constr_t ProgramName = "dbserver";
static constr_t LockfileName = "/home/kerman/projects/GoogleTestC/dbserver.lock";
static constr_t LogfileNamePrefix = "/home/kerman/projects/GoogleTestC/dbserver.log";
static string_t LogfileName[SZ_FULL_PATH_NAME];
static int LogfileFd = -1;

static void validateInput(const int argc, const char **argv, unsigned long * const maxUsers);
...
int main(const int argc, const char **argv)
{
    ...
    // Validate input
    validateInput(argc, argv,  &StartInfo.maxUsers);
    ...
    for (;;)
    {
        ...
        for (int mpx = ListenFdMpx; mpx < StartInfo.maxPoll; mpx++)
        {
            ...
            LogPkgTraceF(LOGPKG_WIRE, "Processing mpx: %d, fd: %d, revents: %d", mpx, MainPoll[mpx].fd, MainPoll[mpx].revents);
...
static void showUsage(void)
{
    fprintf(stderr, "Usage: dbserver maxUsers\n");
...
static void *ProcessorThread(void *arg)
{
    ...
    char *msg = elem->request + startPos;
    ...
    // Request is a 6 character id followed by 6 zeros. In the response, the id
    // is left as is and the 6 zeros are populated by an audit number.
    if (msgLen != SZ_DB_MSG || memcmp(msg + SZ_DB_ID, "000000", SZ_DB_ID))
    {
        elem->closeClient = 1;
        return(endProcessorThread(threadId, elem));
    }
    ...
    // Set up response
    dbmsg_t dbmsg;
    memcpy(&dbmsg, msg, msgLen);

    string_t auditNo[SZ_BANK_AUDIT_NO + 1];
    snprintf(auditNo, sizeof(auditNo), "%0*d", SZ_BANK_AUDIT_NO, ++NextAuditNumber);

    StrToBuf(auditNo, dbmsg.auditNo, SZ_BANK_AUDIT_NO);

    setupResponse(ProgramName, (char *) &dbmsg, sizeof(dbmsg), elem);
